...
async fn test_one_block() -> Result<(), Box<dyn std::error::Error>> {
    let mut storage = MerkleStorage::new(Arc::new(RwLock::new(DB::new())));
    //env!("BASE_URL")
    let base_url = "http://127.0.0.1:18732";

    let block_hash = "BMSYBzugot2iitu7y43LudwHQzPSFxGQgM193ArofiM72A1ZUZc";
    let actions_url = format!("{}/dev/chains/main/actions/blocks/{}?limit=1000000", base_url, block_hash);
